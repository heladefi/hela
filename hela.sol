...
contract Hela is ReentrancyGuard {
    ...
    // Purchase end time  43200  172800
    uint256 public timeEnd = timeStart + 172800;
    // Priority purchase end time  7200
    uint256 public firstTimeEnd = timeStart + 7200;
    ...
    // Number of usdt and tokens to be allocated for lucky prize
    uint256 public luckyUsdt;
    uint256 public luckyToken;
    ...
    function setTimeStart(uint256 _time) external {
        ...
        timeStart = _time;
        timeEnd = timeStart + 172800;
        firstTimeEnd = timeStart + 7200;
    }
    ...
    function purchase() external payable nonReentrant {
        ...
        if(index != 1){
            ...
            // Allocate 50%u to the U pool (25%+2.5%nft+0.5% Xin Yun prize +2% super node +20% exchange token)
            proportion = 50;
        } else {
            // The first order allocates 70% u to the U pool (25%+2.5%nft+0.5% Xin Yun prize +2% super node +40% exchange token)
            proportion = 70;
        }
        ...
        // Contract allocation 5% to restart the bonus pool
        jackpotUsdt += _amount.mul(5).div(100);
        uint256 nftVal = totalToken.mul(250).div(10000);
        nftUsdt += _amount.mul(250).div(10000);
        nftToken += nftVal;
        
        uint256 prizeVal = totalToken.mul(50).div(10000);
        luckyUsdt += _amount.mul(50).div(10000);
        luckyToken += prizeVal;
        TransferHelper.safeTransferFrom(token, allocatedAddress, address(this), nftVal.add(prizeVal));
        ...
        if(timeEnd + 600 > block.timestamp + 172800){
            timeEnd = block.timestamp + 172800;
        } else {
            timeEnd += 600;
        }
    ...
    function queryAuthority() public view returns (bool) {
        if(polling == 0){
            ...
            if(quantity[polling-1] - ordersData[polling-1][msg.sender] <= 50){
                return false;
            }
    ...
    function reset() public nonReentrant {
        ...
        if(index <= 50 && index != 0){
            ...
            for(uint256 i = index-49; i <= index; i++){
                TransferHelper.safeTransfer(usdt, ordersList[polling][i], jackpotUsdt.div(50));
                award memory newData = award({
                    _user:ordersList[polling][i]
                });
                resetAward[polling].push(newData);
            }
            resetBonus[polling] = jackpotUsdt.div(50);
            resetIndex[polling] = index;
        }
    ...
    function getPrice(uint256 _usdtAmount) public view returns (priceData memory) {
        uint256 lastPolling;
        if(polling != 0){
        ...
        require(_usdtAmount < lastUsdtBalance && _usdtAmount < currentUsdtBalance,"Insufficient Balance");
    ...
    function sellToken(uint256 amount) external payable nonReentrant {
        ...
        priceData memory price = getPrice(0);
        TransferHelper.safeTransferETH(feeAddress, fee);
    ...
    event luckyExtractList(uint,uint,uint,uint,address);
    function luckyExtract(address _user) internal {
        TransferHelper.safeTransfer(usdt, _user, luckyUsdt);
        TransferHelper.safeTransfer(token, _user, luckyToken);
        emit luckyExtractList(index,luckyUsdt, luckyToken, block.timestamp, _user);
        luckyUsdt = 0;
        luckyToken = 0;
    }
